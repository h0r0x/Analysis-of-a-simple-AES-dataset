import TRS_Reader  # Module to read the TRS file
import numpy as np  # For numerical calculations
from heapq import nlargest  # To get the top 5 elements
from scipy.stats import pearsonr  # To calculate the Pearson correlation


class AESAttackHammingDistance:
    # Define the AES S-box (SubBytes lookup table)
    sbox = [
        ,
    ]

    def ADK(self, X, k):
        # Perform XOR between input X and key candidate k
        return X ^ k

    def Sbox(self, X):
        # Apply S-box substitution to each byte in X
        return np.array([self.sbox[val] for val in X], dtype=int)

    def hamming_distance(self, X, Y):
        # Compute Hamming Distance between two arrays
        return np.array([bin(x ^ y).count("1") for x, y in zip(X, Y)], dtype=int)

    def maxCorr(self, hd_values, traces):
        # Compute the Pearson correlation for each sample point
        correlations = np.array([pearsonr(hd_values, traces[:, i])[0] for i in range(traces.shape[1])])
        maxcorr = np.max(np.abs(correlations))
        return maxcorr, correlations

    def Initialise(self, N):
        print("Initializing AES attack with Hamming Distance model...")
        trs = TRS_Reader.TRS_Reader("TinyAES_625Samples_FirstRoundSbox.trs")
        trs.read_header()
        trs.read_traces(N, 0, trs.number_of_samples)

        # Prepare arrays to store Hamming Distance predictions
        HDLeakage = np.zeros((16, 256, N), dtype=int)

        # For each byte of the key (16 total bytes)
        for byteno in range(16):
            X = trs.plaintext[:, byteno]  # Get plaintext byte

            for kg in range(256):
                Y_in = self.ADK(X, kg)  # Input to SubBytes
                Y_out = self.Sbox(Y_in)  # Output of SubBytes
                HDLeakage[byteno, kg] = self.hamming_distance(Y_in, Y_out)  # Compute Hamming Distance

        print("Initialization complete.")
        return trs, HDLeakage

    def attackByte(self, trs, HDLeakage, byteno, Nm):
        print(f"Attacking byte {byteno} using {Nm} traces...")
        maxkg = 0  # Most likely key guess
        max_metric = 0  # Maximum correlation metric
        all_metrics = []

        for kg in range(256):
            hd_values = HDLeakage[byteno, kg, :Nm]  # Get Hamming Distance values
            metric, _ = self.maxCorr(hd_values, trs.traces[:Nm, :])  # Calculate correlation
            all_metrics.append((kg, metric))

            if metric > max_metric:
                maxkg = kg
                max_metric = metric

        # Print the top 5 key guesses
        top_5 = nlargest(5, all_metrics, key=lambda x: x[1])
        print(f"Top 5 key guesses for byte {byteno}: {[(hex(k), f'{m:.4f}') for k, m in top_5]}")
        print(f"Recovered key byte {byteno}: 0x{maxkg:02X}")
        return maxkg

if __name__ == "__main__":
    # Initialize the attack
    N_traces_total = 1000  # Total number of traces available
    aes_attack = AESAttackHammingDistance()
    trs, HDLeakage = aes_attack.Initialise(N_traces_total)

    # Perform attack
    Nm = 1000  # Number of traces to use for the attack
    recovered_key = np.zeros(16, dtype=int)

    for byteno in range(16):
        recovered_key[byteno] = aes_attack.attackByte(trs, HDLeakage, byteno, Nm)

    print("\nRecovered key:")
    print(" ".join(f"{byte:02X}" for byte in recovered_key))
