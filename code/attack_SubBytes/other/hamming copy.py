...
class AESAttackSingleBit:
    ...
    def Initialise(self, N):
        print("Initializing AES attack with bit-wise Hamming weight leakage model...")
        ...
        # Prepara un array per memorizzare il Peso di Hamming per bit
        BitHammingLeakage = np.zeros((16, 256, 8, N), dtype=int)
        ...
        for byteno in range(16):
            ...
            for kg in range(256):
                ...
                for bit_pos in range(8):
                    # Maschera per isolare il singolo bit
                    bit_value = (Y >> bit_pos) & 1
                    # Calcola il Peso di Hamming per il singolo bit
                    BitHammingLeakage[byteno, kg, bit_pos] = bit_value

        print("Initialization complete.")
        return trs, BitHammingLeakage


    def attackSingleBitHamming(self, trs, BitHammingLeakage, byteno, Nm, distinguisher="correlation"):
        ...
        for kg in range(256):
            for bit_pos in range(8):
                leakage = BitHammingLeakage[byteno, kg, bit_pos, 0:Nm]  # Ottieni il modello per il bit

                if distinguisher == "correlation":
                    # Calcola la correlazione tra il leakage e le tracce
                    ...
                    # Dividi le tracce in gruppi in base al valore di leakage
                    group0 = trs.traces[0:Nm][leakage == 0, :]
                    group1 = trs.traces[0:Nm][leakage == 1, :]

                    mean0 = np.mean(group0, axis=0) if group0.shape[0] > 0 else 0
                    mean1 = np.mean(group1, axis=0) if group1.shape[0] > 0 else 0

                    metric = np.max(np.abs(mean1 - mean0))
                else:
                    raise ValueError("Invalid distinguisher selected.")

                all_metrics.append((kg, bit_pos, metric))

                # Salva il risultato
                if kg not in key_metrics:
                    key_metrics[kg] = []
                key_metrics[kg].append(metric)

        # Identifica il miglior guess per ciascun bit
        best_kg = max(key_metrics, key=lambda k: np.sum(key_metrics[k]))
        print(f"Most likely key guess: 0x{best_kg:02X}")

        # Stampa i top 5 guess per ciascun bit
        print(f"Top 5 guesses per bit for byte {byteno}:")
        top_5 = sorted(all_metrics, key=lambda x: x[2], reverse=True)[:5]
        for kg, bit, metric in top_5:
            print(f"  Key 0x{kg:02X}, Bit {bit}: Metric = {metric:.4f}")
...
if __name__ == "__main__":
    ...
    for distinguisher in ["correlation", "difference_of_means"]:
        ...
        for Nm in Nm_list:
            ...
            for byteno in range(16):
                recovered_key[byteno] = aes_attack.attackSingleBitHamming(trs, HammingLeakage, byteno, Nm, distinguisher)
