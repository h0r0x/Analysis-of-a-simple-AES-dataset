...
class AESAttackSingleBit:
    ...
    def find_interesting_points(self, hw_values, traces, num_points=5):
        """
    ...
    def Initialise(self, N):
        ...
        for byteno in range(16):
            ...
            for kg in range(256):
                ...
                # Y = self.Sbox(Y)  # Apply S-box transformation
    ...
    def attackBit(self, trs, BitLeakage, byteno, Nm, distinguisher="correlation"):
        ...
        maxkg = np.zeros(8, dtype=int)  # Most likely key guesses for each bit
        max_metric = np.zeros(8)  # Maximum metric for each bit

        all_metrics = [[] for _ in range(8)]  # List to store metrics for each bit
        ...
        for kg in range(256):
            for bit_pos in range(8):
                ...
                # Update if this key guess has a higher metric for this bit
                if metric > max_metric[bit_pos]:
                    maxkg[bit_pos] = kg
                    max_metric[bit_pos] = metric

                # Store all metrics for later sorting
                all_metrics[bit_pos].append((kg, metric))

        # Print the top 5 key guesses for each bit
        print(f"Top 5 key guesses for byte {byteno}:")
        for bit_pos in range(8):
            top_5 = nlargest(5, all_metrics[bit_pos], key=lambda x: x[1])  # Get top 5 by metric
            print(f"  Bit {bit_pos}: {[(hex(k), f'{m:.4f}') for k, m in top_5]}")

        # Determine the most frequent key guess across all bits
        key_guess_counts = {}

        # Count occurrences of top-1 key guesses for each bit
        for bit_pos in range(8):
            top_1_key = max(all_metrics[bit_pos], key=lambda x: x[1])[0]  # Take the top-1 key
            key_guess_counts[top_1_key] = key_guess_counts.get(top_1_key, 0) + 1

        # Convert keys to hexadecimal for readability
        key_guess_counts_hex = {hex(k): v for k, v in key_guess_counts.items()}
        print(f"Key guesses frequency for byte {byteno}: {key_guess_counts_hex}")

        # Select the key guess that appears most frequently
        most_frequent_key = max(key_guess_counts, key=key_guess_counts.get)
        most_frequent_count = key_guess_counts[most_frequent_key]

        print(f"Most frequent key guess: {hex(most_frequent_key)} (appears {most_frequent_count} times)")

        # Use the most frequent key as the final guess for the byte
        recovered_key_byte = most_frequent_key
...
if __name__ == "__main__":
    ...
    for distinguisher in ["correlation"]:
        ...
        for Nm in Nm_list:
            ...
            print(f"Recovered key ({Nm} traces):")
