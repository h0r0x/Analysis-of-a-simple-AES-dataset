...
class AESAttack(object):
    ...
    def maxCorr(self, hw, traces):
        ...
        for i in range(traces.shape[1]):  # Usa la dimensione delle tracce filtrate
    ...
    def corrAttack(self, trs, ax, byteno, Nm, distinguisher="maxCorr"):
        """
    ...
    def identify_leakage(self, traces, hw):
        """
        Identifica i punti di leakage significativi nelle tracce.
        """
        
        leakage_points = []
        for i in range(traces.shape[1]):
            group0 = traces[hw == 0, i]
            group1 = traces[hw == 1, i]
            t_stat, p_value = ttest_ind(group0, group1, equal_var=False)
            if p_value < 0.05:  # Soglia di significatività
                leakage_points.append(i)
        return leakage_points


if __name__ == "__main__":
    # Initialize the attack
    print("Starting the CPA attack with leakage analysis...")
    [trs, HWguess] = AESAttack().Initialise(1000)

    # Identifica i punti di leakage per un byte (es. il primo)
    print("Identifying leakage points...")
    leakage_points = AESAttack().identify_leakage(trs.traces, HWguess[0, 0])
    print(f"Leakage points identified: {leakage_points[:10]} (showing first 10, total: {len(leakage_points)})")

    if not leakage_points:
        print("Warning: No significant leakage points identified. Using all samples.")
        leakage_points = list(range(trs.traces.shape[1]))  # Usa tutti i campioni
    ...
    for i in range(10):  # Plotta 10 tracce rappresentative
        plt.plot(trs.traces[i, leakage_points] + i * 0.05, label=f"Trace {i+1}", alpha=0.8)  # Offset per separarli
    plt.title("Example Traces (Filtered by Leakage Points)")
    plt.xlabel("Sample Index (Filtered)")
    plt.ylabel("Power Consumption (arbitrary units)")
    plt.legend()
    plt.tight_layout()
    plt.savefig("images/example_traces_filtered.png", dpi=300, bbox_inches="tight")
    print("Filtered traces plot saved as 'example_traces_filtered.png'.")
    ...
    for byteno in range(16):
        ...
        for kg in range(256):
            hw = HWguess[byteno, kg]
            try:
                maxcorr, corr = AESAttack().maxCorr(hw[:Nm], trs.traces[:Nm, :][:, leakage_points])  # Usa campioni filtrati
            except IndexError as e:
                print(f"Error in maxCorr for key guess {kg}: {e}")
                continue
        ...
        for kg, score in key_guesses[:5]:  # Colora i top 5 in modo distintivo
            hw = HWguess[byteno, kg]
            _, corr = AESAttack().maxCorr(hw[:Nm], trs.traces[:Nm, :][:, leakage_points])  # Usa campioni filtrati
            ax[byteno].plot(corr, label=f"Key 0x{kg:02X}", alpha=0.8)

        # Configurazione sottografico
        ax[byteno].set_xlim([0, len(leakage_points)])  # Aggiornato in base ai campioni filtrati
        ax[byteno].set_ylim([-1, 1])
        ax[byteno].set_title(f"Byte {byteno}: Key 0x{key_guesses[0][0]:02X}")
        ax[byteno].set_xlabel("Samples (Filtered)")
        ax[byteno].set_ylabel(r"$\rho$")
        ax[byteno].legend()

    # Salva l'immagine
    plt.tight_layout()
    plt.savefig("images/max_correlation_attack_filtered.png", dpi=300, bbox_inches="tight")
    print("Max correlation attack plot (filtered) saved as 'max_correlation_attack_filtered.png'.")
